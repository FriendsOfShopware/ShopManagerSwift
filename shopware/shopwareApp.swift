...
struct shopwareApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            #if DEBUG
            if ProcessInfo.processInfo.arguments.contains("--setup-ui-fixtures") {
                SetupUITestRoot().tint(Theme.accent)
            } else if ProcessInfo.processInfo.arguments.contains("--navigation-ui-fixtures") {
