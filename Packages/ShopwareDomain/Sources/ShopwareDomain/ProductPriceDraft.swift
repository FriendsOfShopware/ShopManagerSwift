...
public struct ProductPriceDraft: Equatable, Sendable {
    public let currencyID: String
    public var gross: String
    public var net: String
    public var linked: Bool
    public init(price: JSONValue?, currencyID: String = ShopwareDefaults.currencyID, locale: Locale) {
    ...
    public mutating func updateGross(_ value: String, taxRate: Double?, locale: Locale) {
        gross = value
    ...
    public mutating func updateNet(_ value: String, taxRate: Double?, locale: Locale) {
        net = value
        if linked, let taxRate, let value = ProductNumberInput.decimal(value, locale: locale) {
            gross = ProductNumberInput.text(value * (1 + taxRate / 100), locale: locale)
        }
    }
    public func validationError(locale: Locale) -> String? {
        guard ProductNumberInput.decimal(gross, locale: locale) != nil, ProductNumberInput.decimal(net, locale: locale) != nil else {
            return Bundle.module.localizedString(forKey: "Enter valid gross and net prices of zero or greater.", value: nil, table: nil)
        }
        return nil
    }
    ...
    public func prices(replacing original: [JSONValue], locale: Locale) -> [JSONValue]? {
